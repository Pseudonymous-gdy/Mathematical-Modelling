        ...
        for bs_idx in range(len(state)):
            for slice_idx in range(3):
                ...
                if (total_url <= len(URLLC_USERS) and
                    ...
                    # 分别判断每个基站的RB限制
                    rb_limits = [RB_LIMIT] * (len(new_state)-1) + [100] if len(new_state) > 1 else [RB_LIMIT]
...
def compute_sinr_linear(
    ...
    I = 0.0
    # If BS4 is serving, it is not affected by interference from others.
    if bs_serv.bs_id != 4:
        for (bs_i, pl_i_db, ray_i_db) in interferers:
            # BS4 does not cause interference to others.
            if bs_i.bs_id == 4:
                continue
...
def load_all_data(base_dir: Path):
    ...
    bs_data = {}
    for bs_id in [1, 2, 3, 4]:
        xls = pd.ExcelFile(base_dir / fr"D:\2025年第六届华数杯数学建模竞赛赛题\B题\附件\附件4\BS{bs_id}.xlsx")
    ...
    tf_xls = pd.ExcelFile(base_dir / r"D:\2025年第六届华数杯数学建模竞赛赛题\B题\附件\附件4\taskflow.xlsx")
...
def schedule_all_rounds_multi_bs(num_rounds=10, p_init=(30, 30, 30, 30)):
    ...
    BS = {
        ...
        4: BaseStation(4, "BS4", tx_power_dbm=float(p_init[3]), rb_total=100),
    }
    ...
    for r in range(num_rounds):
        ...
        tasks_bs = {b: tasks_win[['ms']].copy() for b in [1,2,3,4]}
        for u in ALL_USERS:
        ...
        slice_qos_parts = {}
        for b in [1, 2, 3, 4]:
            other_bses = [bb for bb in [1,2,3,4] if bb != b]
        ...
        cand_p = [
            ...
            neighborhood(p_prev[3], 10, 40), # BS4 has a different power range
        ]
            ...
            for p2 in cand_p[1]:
                for p3 in cand_p[2]:
                    for p4 in cand_p[3]:
                        ...
                        BS[1].tx_power_dbm, BS[2].tx_power_dbm, BS[3].tx_power_dbm, BS[4].tx_power_dbm = float(p1), float(p2), float(p3), float(p4)
        ...
        BS[1].tx_power_dbm, BS[2].tx_power_dbm, BS[3].tx_power_dbm, BS[4].tx_power_dbm = map(float, p_curr)

        # Final simulation to produce leftovers & slice QoS with chosen powers
        total_qos_final = 0.0
        inherited_final = {1: [], 2: [], 3: [], 4: []}
...
if __name__ == "__main__":
    # Example run: 10 rounds
    results = schedule_all_rounds_multi_bs(num_rounds=10, p_init=(30,30,30,40))
    for r in results:
        print(f"Round {r['round']}: "
              f"BS1 part={r['partition_BS1']}, BS2 part={r['partition_BS2']}, BS3 part={r['partition_BS3']}, BS4 part={r['partition_BS4']}; "
              f"p=({r['p_BS1']},{r['p_BS2']},{r['p_BS3']},{r['p_BS4']}), "
